...
def home(request):
    ...
    video = OtelMemnuniyet.objects.filter(video_yayinla=True)
    vid = VideoGallery.objects.all()
    carousel = Content.objects.first()
    
    return render(request, 'index.html', {'yorumlar': yorumlar, 'galeri': galeri, 'videolar' : video, 'carousel': carousel, 'vid': vid})
